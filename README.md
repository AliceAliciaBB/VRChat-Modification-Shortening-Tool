...
3. **③ アイテム追加** — 格納先を選んでプレハブをドロップし、「追加するプレハブのオンオフ」(衣装/髪型/作成しない)を選んで「追加」を押します。
   ...
   - プレハブにもともとメニューインストーラー(VRCExpressionsMenu/ModularAvatarMenuInstaller)が付いていた場合は検出され、ダイアログで確認した上で `M_<name>` 配下へ統合できます(自動統合はしません)。
...
Unityプロジェクト内のパッケージ(フォルダ群)を別の場所へ移動するためのユーティリティウィンドウです。Unityメニューの **ALICILIA** から開けます。

### PackageMover ウィンドウ

パッケージ全体(複数フォルダ)を移動先へ一括移動します。移動先のフォルダ構造は「`Assets/<DestRoot>/<PackageName>/`」または「`Assets/<PackageName>/`」の2パターンを自動判定します。同名フォルダが既に存在する場合はマージします。

### Folder Move ウィンドウ

Projectウィンドウで**左クリックで選択したフォルダ単体**を移動します。選択フォルダの元カテゴリ直下から本体までの中間階層だけを移動先に再現し、フォルダ本体(子要素含む)のみを移動します。パッケージ全体ではなく特定フォルダだけを別カテゴリへ移したい場合に使います。
