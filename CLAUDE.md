...
- `会議/` フォルダ: 対話モードのみで説明しきれない検討事項をtxtで直書きするための置き場。サブフォルダは `会議前`(議題・たたき台)/`会議中`(その場のメモ)/`会議決定事項`(確定した結論)/`過去のドキュメント`(実装完了後は基本放置、実態と乖離してもよい)。空フォルダ維持用に各フォルダへ`.gitkeep`を配置済み。
